...
- "Which GPUs does Afterburner see?"
...
The full 18-function surface behind these prompts: monitoring reads (`get_gpu_status`,
`list_gpus`, `get_gpu_limits`, `get_tuning_state`, `get_profiles`, `show_configuration`,
...
`set_memory_offset`, `set_fan_percent`, `set_fan_auto`, `set_fan_curve`, `optimize_quiet`,
`optimize_thermal`), and evidence-based diagnostics (`diagnose_performance`).
